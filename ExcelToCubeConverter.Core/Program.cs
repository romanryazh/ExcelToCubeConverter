...
try
{
	Console.WriteLine("Процесс конвертации...");
...
List<FieldInfo> ParseExcel(ExcelWorksheet worksheet)
{
	...
	{
		var cubeName = worksheet.Cells[row, 1].Text.Trim();
		...
		{
			CubeName =  cubeName,
			FieldName = worksheet.Cells[row, 2].Text.Trim(),
			SqlColumn = cubeName,
			DataType =  worksheet.Cells[row, 3].Text.Trim(),
			IsMetric =  worksheet.Cells[row, 4].Text.Trim() == "Metric",
			Aggregation =  worksheet.Cells[row, 5].Text.Trim().ToUpper(),
		};
